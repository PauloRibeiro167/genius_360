...
RSpec.describe "action_permissions/index", type: :view do
  let(:records) { create_list(:action_permission, 2) }

  before(:each) do
    records
    @q = ActionPermission.ransack
    assign(:action_permissions, @q.result.page(1))
  end

  it "exibe o título da página" do
    render
    expect(rendered).to have_content("Lista de action_permissions")
  end

  it "exibe a tabela com os registros" do
    render
    expect(rendered).to have_selector("table.table")
    expect(rendered).to have_selector("tr", count: ActionPermission.count + 2) # +2 para cabeçalho e filtros
  end
  ...
  it "exibe o botão de novo registro" do
    render
    expect(rendered).to have_link("Novo ActionPermission", href: new_action_permission_path)
  end
