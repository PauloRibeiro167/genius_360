...
RSpec.describe "controller_permissions/index", type: :view do
  let(:records) { create_list(:controller_permission, 2) }

  before(:each) do
    records
    @q = ControllerPermission.ransack
    assign(:controller_permissions, @q.result.page(1))
  end

  it "exibe o título da página" do
    render
    expect(rendered).to have_content("Lista de controller_permissions")
  end

  it "exibe a tabela com os registros" do
    render
    expect(rendered).to have_selector("table.table")
    expect(rendered).to have_selector("tr", count: ControllerPermission.count + 2) # +2 para cabeçalho e filtros
  end
  ...
  it "exibe o botão de novo registro" do
    render
    expect(rendered).to have_link("Novo ControllerPermission", href: new_controller_permission_path)
  end
