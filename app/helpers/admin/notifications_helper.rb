module Admin::NotificationsHelper
  def admin_notification_status_badge(admin_notification)
    if admin_notification.discarded?
  ...
  def admin_notification_actions(admin_notification)
    links = []
    
    links << link_to(admin_notification, class: "text-dark") do
      content_tag(:i, "", class: "fa-regular fa-eye")
    end

    links << link_to(edit_admin_notification_path(admin_notification), class: "text-warning") do
      content_tag(:i, "", class: "fa-regular fa-edit")
    end

    if admin_notification.discarded?
      links << link_to(undiscard_admin_notification_path(admin_notification),
        class: "text-success",
        data: {
      ...
      links << link_to(discard_admin_notification_path(admin_notification),
        class: "text-danger",
        data: {
          controller: "sweetalert",
          action: "click->sweetalert#confirmAlert",
